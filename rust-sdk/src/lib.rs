...
//! # Key Features
//!
//! - **No hand-written generated code** -- Proto stubs are compiled
//!   automatically at build time (requires `protoc`) and re-exported from this
//!   crate so plugin authors never check in generated files.
...
pub trait PluginHandler: Send + Sync + 'static {
    ...
    /// Decide per-request whether admin authentication is required.
    ///
    /// KBS calls this before enforcing authentication and before [`handle`](Self::handle).
    /// Return `requires_admin_auth = true` to require admin credentials for this
    /// request; return `false` (the default) to require an attestation token and
    /// policy evaluation.
    ///
    /// Override to implement per-request routing — for example, `POST` requests
    /// that provision secrets use admin auth while `GET` requests that retrieve
    /// secrets use attestation.
    ///
    /// # Default behaviour
    ///
    /// Returns `requires_admin_auth = false` (attestation-gated) for every request.
    /// Note: this is an active `false` return, not a protocol-level `UNIMPLEMENTED`.
    /// If your KBS config sets `validate_auth = true` for this plugin but you do not
    /// override this method, KBS will still use the attestation-gated path because
    /// the SDK intercepts the RPC before KBS can apply the config fallback.
    /// Override this method whenever you need admin authentication for any request.
    async fn validate_auth(
        &self,
        _request: Request<ValidateAuthRequest>,
    ...
    /// Decide per-request whether to JWE-encrypt the response.
    ///
    /// KBS calls this after [`handle`](Self::handle) returns. Return `encrypt = true`
    /// to JWE-encrypt the response body with the TEE's ephemeral public key (required
    /// for secret material). Return `false` (the default) for non-sensitive responses.
    ///
    /// JWE encryption is only available on the attestation-gated path; this method
    /// is never called for requests that `validate_auth` routes to admin auth.
    ///
    /// # Default behaviour
    ///
    /// Returns `encrypt = false`. Override for endpoints that return secret material.
...
mod tests {
    ...
    #[tonic::async_trait]
    impl PluginHandler for MinimalPlugin {
        async fn handle(
            &self,
        ...
        async fn capabilities(&self) -> CapabilitiesBuilder {
            CapabilitiesBuilder::new("test", "1.0.0")
        }
    }

    #[tokio::test]
    async fn validate_auth_default_is_attestation_gated() {
        let plugin = MinimalPlugin;
        let request = tonic::Request::new(ValidateAuthRequest {
            body: vec![],
        ...
        let response = plugin
            ...
            .await
            .expect("default validate_auth never returns Err");
        assert!(
            !response.into_inner().requires_admin_auth,
            "default should be attestation-gated (false)"
        );
    }

    #[tokio::test]
    async fn needs_encryption_default_is_false() {
        let plugin = MinimalPlugin;
        let request = tonic::Request::new(NeedsEncryptionRequest {
            body: vec![],
            query: Default::default(),
            path: vec![],
            method: "GET".into(),
        ...
        let response = plugin
            ...
            .await
            .expect("default needs_encryption never returns Err");
        assert!(!response.into_inner().encrypt, "default should not encrypt");
    }
}
