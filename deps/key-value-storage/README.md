...
The `StorageBackendConfig` provides a unified way to configure storage backends that can be shared across multiple components in an application. This is particularly useful for applications like KBS (Key Broker Service) that need to manage multiple storage instances for different purposes.

### What is an Instance?

An **instance** is a logical separation of data within the same storage backend. When using a unified storage backend configuration, different components of an application can share the same storage backend type and configuration, but their data is stored separately using different instance names.

For example:
- With `LocalFs` backend: Different instances are stored in different subdirectories
- With `LocalJson` backend: Different instances use different JSON file names
- With `Postgres` backend: Different instances use different table names
...
| Property | Type | Description | Required | Default |
|----------|------|-------------|----------|---------|
| `dir_path` | String | Base directory path for all storage instances. Different instances will be stored in subdirectories under this path. | No | `/opt/confidential-containers/storage/local_fs` |

**Example:**
- Instance `"kbs"` → stored in `<dir_path>/kbs/`
- Instance `"resource"` → stored in `<dir_path>/resource/`
...
| Property | Type | Description | Required | Default |
|----------|------|-------------|----------|---------|
| `file_dir_path` | String | Base directory path for all JSON files. Different instances will use different file names under this directory. | No | `/opt/confidential-containers/storage/local_json` |

**Example:**
- Instance `"kbs"` → stored in `<file_dir_path>/kbs.json`
- Instance `"resource"` → stored in `<file_dir_path>/resource.json`
...
**Example:**
- Instance `"kbs"` → stored in table `kbs`
- Instance `"resource"` → stored in table `resource`
...
When `storage_type = "Memory"`, no additional configuration is needed. Each instance will have its own in-memory storage that is not persisted.
