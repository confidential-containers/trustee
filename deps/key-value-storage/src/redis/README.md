...
If the `REDIS_URL` environment variable is set, it takes precedence over the `url` from the configuration file. This allows a connection URL that embeds credentials to be injected from a secret store (e.g. a Kubernetes Secret) instead of being written into a config file.
