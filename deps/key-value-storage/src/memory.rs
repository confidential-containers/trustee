...
impl KeyValueStorage for MemoryKeyValueStorage {
    #[instrument(skip_all, name = "MemoryKeyValueStorage::set")]
    ...
    #[instrument(skip_all, name = "MemoryKeyValueStorage::get")]
    async fn get(&self, key: &str) -> Result<Option<Vec<u8>>> {
        let res = self.items.read().await.get(key).cloned();
        Ok(res)
    }

    #[instrument(skip_all, name = "MemoryKeyValueStorage::delete")]
