...
This module provides a PostgreSQL-backed implementation of `KeyValueStorage` for the key-value storage system. Key-value pairs are stored in a single table with the value content base64-url encoded to prevent SQL injection and ensure safe storage of arbitrary text.

## Features

- Stores key-value pairs in a configurable table (`key` primary key, `value` as base64-url encoded text)
...
What it does:
- Starts a container named `policy-postgres` on host port `6432`
