...
static BIOS_SUBTYPE_HANDLERS: LazyLock<HashMap<u8, Box<dyn DeviceSubTypeParser + Send + Sync>>> =
    LazyLock::new(|| {
        let mut handlers: HashMap<u8, Box<dyn DeviceSubTypeParser + Send + Sync>> = HashMap::new();
        handlers.insert(0x01, Box::new(BbsParser));
...
impl DevicePathParser for BiosBootSpecParser {
    fn parse(&self, sub_type: u8, data: &[u8]) -> Result<String> {
        BIOS_SUBTYPE_HANDLERS
            .get(&sub_type)
            .ok_or_else(|| {
                anyhow::anyhow!("Unknown Bios Boot Specification subtype: {:#04x}", sub_type)
            })?
            .parse(data)
    }
}
...
impl DeviceSubTypeParser for BbsParser {
    fn parse(&self, data: &[u8]) -> Result<String> {
        ...
        let description = std::str::from_utf8(&desc_bytes[..desc_end])
