...
static MEDIA_SUBTYPE_HANDLERS: LazyLock<HashMap<u8, Box<dyn DeviceSubTypeParser + Send + Sync>>> =
    LazyLock::new(|| {
        let mut handlers: HashMap<u8, Box<dyn DeviceSubTypeParser + Send + Sync>> = HashMap::new();
        handlers.insert(0x01, Box::new(HardDriveParser));
        handlers.insert(0x03, Box::new(MediaVendorParser));
        handlers.insert(0x04, Box::new(FilePathParser));
...
impl DevicePathParser for MediaParser {
    fn parse(&self, sub_type: u8, data: &[u8]) -> Result<String> {
        MEDIA_SUBTYPE_HANDLERS
            .get(&sub_type)
            .ok_or_else(|| anyhow!("Unknown Media subtype: {:#04x}", sub_type))?
            .parse(data)
    }
}
