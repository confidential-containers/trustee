...
static MESSAGING_SUBTYPE_HANDLERS: LazyLock<
...
> = LazyLock::new(|| {
    ...
    handlers.insert(0x15, Box::new(FibreChannelExParser));
    handlers.insert(0x05, Box::new(UsbParser));
    handlers.insert(0x0B, Box::new(MacParser));
    handlers.insert(0x0C, Box::new(Ipv4Parser));
...
impl DevicePathParser for MessagingParser {
    fn parse(&self, sub_type: u8, data: &[u8]) -> Result<String> {
        MESSAGING_SUBTYPE_HANDLERS
            .get(&sub_type)
            .ok_or_else(|| anyhow!("Unknown Messaging subtype: {:#04x}", sub_type))?
            .parse(data)
    }
}
