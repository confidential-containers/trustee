...
impl Serialize for EventlogEntry {
        ...
        S: Serializer,
    {
        ...
        state.serialize_field("type_name", &self.event_type.to_string())?;
        state.end()
    }
}
...
impl CcEventLog {
    pub fn replay_and_match(&self, data: Vec<ReferenceMeasurement>) -> Result<()> {
        for item in data.iter() {
            let calculated_ccel_rtmr = replay(&self.log, item.index, item.algorithm)?;
...
fn replay(ccel: &[EventlogEntry], index: u32, alg: TcgAlgorithm) -> Result<Vec<u8>> {
    let digest_size = alg.get_digest_size(alg)?;
    let mut materials = vec![0u8; digest_size];

    for entry in ccel.iter().filter(|e| e.index == index) {
        for digest in entry.digests.iter().filter(|d| d.alg == alg) {
