...
impl Rtmr {
    pub fn integrity_check(&self, rtmr_from_quote: HashMap<u32, Vec<u8>>) -> Result<()> {
        let empty = &Vec::from([0u8; RTMR_LENGTH_BY_BYTES]);
        for key in self.data.keys() {
            let quote_value = rtmr_from_quote.get(key).unwrap_or(empty);

            let ccel_value = self.data.get(key).unwrap_or(empty);
            if ccel_value != quote_value {
                bail!(
                    "CCEL eventlog does not pass check. CCEL value: {}, Quote value: {}",
...
mod tests {
    ...
    fn test_rtmr(
        ...
        expected.insert(1, hex::decode(rtmr1).unwrap());
        expected.insert(2, hex::decode(rtmr2).unwrap());
        expected.insert(3, hex::decode(rtmr3).unwrap());
        expected.insert(4, hex::decode(rtmr4).unwrap());

        assert!(rtmr_result.is_ok());
        let integrity = rtmr_result.expect("Result is ok").integrity_check(expected);
