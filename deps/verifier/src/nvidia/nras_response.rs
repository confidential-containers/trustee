...
impl NrasGpuCertChainClaims {
    /// Verify nested cert-chain fields with clear success criteria from the
    /// GPU Claims v3.0 guide / `ear_default_policy_gpu.rego`.
    /// Present fields must match expected values; missing fields are traced.
    fn verify(&self, claim_name: &str) -> Result<()> {
        match &self.x_nvidia_cert_ocsp_status {
            ...
            Some(status) if status == "revoked" => {
                ...
                bail!("{claim_name}.x-nvidia-cert-ocsp-status revoked")
            }
            Some(status) => {
        ...
        if let Some(expiration_date) = &self.x_nvidia_cert_expiration_date {
            if let Ok(date) = DateTime::parse_from_rfc3339(expiration_date) {
                if date < Utc::now() {
...
impl NrasIndividualClaims {
    ...
    /// - Missing fields are logged at trace level and
    /// skipped (they are not treated as failures).
    pub fn verify(&self) -> Result<()> {
        ...
        check_true("x-nvidia-gpu-arch-check", &self.x_nvidia_gpu_arch_check)?;

        check_true("secboot", &self.secboot)?;
        check_str_eq("dbgstat", &self.dbgstat, "disabled")?;
        ...
        check_str_eq("measres", &self.measres, "success")?;

        if self.x_nvidia_gpu_attestation_report_nonce_match != Some(true) {
...
impl NrasSwitchIndividualClaims {
    ...
    pub fn verify(&self) -> Result<()> {
        ...
        if self.x_nvidia_switch_attestation_report_nonce_match != Some(true) {
            bail!("Report Data Mismatch");
        }
        Ok(())
    }
...
fn check_cert_chain(name: &str, chain: &Option<NrasGpuCertChainClaims>) -> Result<()> {
    match chain {
        Some(claims) => claims.verify(name),
        None => {
...
fn check_true(name: &str, value: &Option<bool>) -> Result<()> {
    match value {
        Some(true) => Ok(()),
        Some(false) => bail!("{name} check failed"),
        None => {
            trace!("{name} is missing");
            Ok(())
        }
...
fn check_str_eq(name: &str, value: &Option<String>, expected: &str) -> Result<()> {
    match value {
        Some(v) if v == expected => Ok(()),
        Some(v) => bail!("{name} expected \"{expected}\", got \"{v}\""),
        None => {
            trace!("{name} is missing");
            Ok(())
        }
