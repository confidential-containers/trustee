...
pub(crate) fn parse_tee_evidence(report: &AttestationReport) -> TeeEvidenceParsedClaim {
    let claims_map = json!({
        ...
        // measurements
        "measurement": serde_json::Value::String(STANDARD.encode(report.measurement)),
        "report_data": serde_json::Value::String(STANDARD.encode(report.report_data)),
        "init_data": serde_json::Value::String(STANDARD.encode(report.host_data)),
    });
