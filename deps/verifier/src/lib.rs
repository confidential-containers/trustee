...
///
/// `storage` is a namespace provider already scoped to the verifier subsystem:
/// a verifier that needs to persist state (e.g. cached DCAP collateral) mints
/// its own namespace in-place with
/// `storage.get_or_register("intel-dcap-collateral")` without being able to
/// reach any other component's storage. Verifiers that don't need storage
/// simply ignore it.
