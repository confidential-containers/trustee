...
4. **Response**: The KBS returns a `Response` payload to a KBC requesting a
   resource if and only if the `Attestation` payload was succesfully validated.
...
The KBS matches the attestation evidence to an attesation challenge with the
...
Upon succcessful attestation, the KBC can request resources from the KBS, by
...
// tag is optional and depend on the encryption algorithm.
...
value as describe in the [`Attestation`](#attestation)'s `tee-pubkey` field.
...
The encrypted symmetric key used to encrypt `ciphertext`.
...
key pair.
This field follow the [JSON Web Key](https://www.rfc-editor.org/rfc/rfc7517)
format:
...
attester must authenticate itself against the remote KBS. Only after succesfully
...
2. `/kbs/v0/attest` only accepts `POST` requests whose body is a [KBS Attestation](#attestation)
   ...
   (no content), which HTTP status indicates if the attestation was succesful or
   not.
