...
$ docker logs trustee-as-1 | grep cache_dir
                            cache_dir: "/etc/test-vcek-cache",
```

Then copy your archive (making sure to add the ending `/` in both paths):
```
docker cp ./tools/cache-preloader/vcek_cache/ trustee-as-1:/etc/test-vcek-cache/
```

- **Build into image**

You may also copy the cache as a part of building the image by updating the
attestation service's Docker file:

```
+++ b/attestation-service/docker/as-grpc/Dockerfile
@@ -63,6 +63,7 @@ RUN if [ "${VERIFIER}" = "all-verifier" ] || [ "${VERIFIER}" = "az-snp-vtpm-veri
...
+COPY ./attestation-service/vcek_cache /etc/test-vcek-cache
```

In the above example, the `cache-preloader` tool's target directory was `../../attestation-service/vcek_cache`:
```
cargo run -- -u test_urls.txt -c ../../attestation-service/vcek-cache
```

And `as-config.json` sets `cache_dir` to `/etc/test-vcek-cache`.
...
docker cp ./vcek_cache/ trustee-as-1:/etc/test-vcek-cache/
```
