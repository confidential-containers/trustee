...
use policy_engine::rego::ExtensionWrapper;
...
impl EarAttestationTokenBroker {
    pub async fn new(config: EarTokenConfiguration) -> Result<Self> {
        ...
        let policy_engine = PolicyEngine::<Regorus>::new(config.policy_engine.clone()).await?;
...
mod tests {
    ...
    use key_value_storage::local_fs;
    use policy_engine::KeyValueStorageConfig;
    use serde_json::json;
    use std::io::Write;
    ...
    async fn test_issue_ear_ephemeral_key() {
        ...
        config.policy_engine.storage = KeyValueStorageConfig::LocalFs(local_fs::Config {
            dir_path: "./tests/coco-as/policy/opa".into(),
        });
