...
/// Namespace prefix granted to verifiers. A verifier mints its own
/// sub-namespaces under this scope and cannot reach any other component's
/// storage.
const VERIFIER_STORAGE_SCOPE: &str = "verifier";
    ...
    pub async fn evaluate(
        ...
        for verification_request in verification_requests {
            let verifier = verifier::to_verifier(
                ...
                key_value_storage::scoped(self.storage_provider.clone(), VERIFIER_STORAGE_SCOPE),
            )
            .await?;
