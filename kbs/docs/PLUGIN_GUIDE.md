...
Every plugin implements at least two handler methods (`handle` and `capabilities`). Override `validate_auth` and `needs_encryption` for custom auth routing and response encryption — the SDK provides safe defaults if you don't.
...
> **`validate_auth` and `needs_encryption`**: KBS calls these gRPC operations before and after
> `handle` respectively. The SDK's `PluginHandler` trait provides default implementations for
> both — `validate_auth` defaults to attestation-gated (`requires_admin_auth = false`), and
> `needs_encryption` defaults to no encryption (`encrypt = false`). Override them in your
> implementation to customise auth routing and response encryption. See
> [`rust-sdk/examples/echo.rs`](../../rust-sdk/examples/echo.rs) for a complete working example
> showing all four methods with per-request logging.
...
returns 405 Method Not Allowed without invoking the plugin's `Handle` RPC.
Note that `ValidateAuth` may still be called as part of request processing
before the method check takes effect.
...
Note: KBS returns 405 Method Not Allowed automatically when the request
method is not in `supported_methods`. In this case KBS does not invoke the
plugin's `Handle` RPC, although `ValidateAuth` may still be called as part
of request processing before the method check takes effect.
...
Build your plugin as a static or dynamically linked binary, then package it
in a minimal container image. The plugin binary only needs to listen on the
configured port and implement the `kbs.plugin.v1.KbsPlugin` gRPC service.
...
Fix: Choose a unique `plugin_name` that does not conflict with reserved KBS
endpoint names (`auth`, `attest`, `attestation-policy`, `reference-value`,
`resource-policy`) or other external plugins.
...
the `KbsPlugin` service with four RPCs:
...
The Rust SDK ships an echo plugin that demonstrates all four handler methods
(`handle`, `capabilities`, `validate_auth`, and `needs_encryption`) with
