...
|-----------------------|--------------|-----------------------------------------------------------------------------------------------------------------------------------|---------|
| `trusted_jwk_sets`    | String Array | Trusted JWKS sources. `file://` and local paths: JWKS JSON file. `https://`: JWKS URL or OpenID issuer base URL; loads JWKS directly when possible, otherwise via OpenID discovery. | Empty   |
