...
- `type = "LocalFs"` plus `dir_path` (each value stored as a file, default `/opt/confidential-containers/storage/local_fs`)
- `type = "LocalJson"` plus `file_path` (all values in one JSON file, default `/opt/confidential-containers/storage/local_json/key_value.json`)
...
- `type = "Memory"` (default, in‑memory and volatile)
- `type = "LocalJson"` with `file_path` (single JSON file, default `/opt/confidential-containers/storage/local_json/key_value.json`)
- `type = "LocalFs"` with `dir_path` (one file per policy, default `/opt/confidential-containers/storage/local_fs`)
...
When `backend = "kvstorage"` (default), remaining fields follow [KeyValueStorage][4]:
- `type = "Memory"` (default, in‑memory)
- `type = "LocalFs"` with optional `dir_path` (defaults to `/opt/confidential-containers/kbs/repository`)
- `type = "LocalJson"` with optional `file_path`
