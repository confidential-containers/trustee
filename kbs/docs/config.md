...
- `mode = "DenyAll"`
- `mode = "TokenAuthorization"` (recommended for production)

For `mode = "TokenAuthorization"`, configure:

- `[admin.token_verifier]` with `type = "BearerJwt"`
- `[admin.authorizer]` with `type = "RegexAcl"`

`BearerJwt` properties:

| Property | Type | Description | Required | Default |
|----------|------|-------------|----------|---------|
| `idps` | Array | Trusted issuer entries for JWT verification | No | Empty |
...
| Property | Type | Description | Required |
|----------|------|-------------|----------|
| `issuer` | String | Expected JWT `iss` value | Yes |
...
Security note: keep `insecure_public_key_from_uri=false` whenever possible.
Using `http://` allows network attackers to tamper with downloaded public keys.

`RegexAcl` properties:

| Property | Type | Description | Required |
|----------|------|-------------|----------|
| `acls` | Array | Audience-to-endpoint allow rules | No |

Each ACL entry:

| Property | Type | Description | Required |
|----------|------|-------------|----------|
| `audience` | String | JWT `aud` value to match | Yes |
...
[admin.authorizer]
type = "RegexAcl"
acls = [{ audience = "admin", allowed_endpoints = "^/kbs/.+$" }]
