...
impl KbsPlugin for EchoPlugin {
    ...
    // POST (provision/write) requires admin auth; GET (retrieve) is
    // attestation-gated. Mirrors the pattern used by built-in plugins
    // such as resource and pkcs11.
    async fn validate_auth(
        &self,
        request: Request<ValidateAuthRequest>,
    ) -> Result<Response<ValidateAuthResponse>, Status> {
        let requires_admin_auth = request.into_inner().method == "POST";
    ...
    // Mirrors the sample built-in plugin which also returns false.
