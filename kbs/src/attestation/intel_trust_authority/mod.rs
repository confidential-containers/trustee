...
use crate::{
    attestation::backend::{generic_generate_challenge, make_nonce, Attest, TeeClass, TeeEvidence},
...
impl Attest for IntelTrustAuthority {
    async fn verify(
        ...
        let runtime_data = evidence_to_verify[0].3.to_string();
        let tee = evidence_to_verify[0].0;
        ...
        let (req_data, att_url) = match tee {
            Tee::AzTdxVtpm => {
                ...
                let evidence = from_value::<AzItaTeeEvidence>(evidence_to_verify[0].2.clone())
            ...
            Tee::Tdx | Tee::Sgx => {
                ...
                let evidence = from_value::<ItaTeeEvidence>(evidence_to_verify[0].2.clone())
                    .context(format!("Failed to deserialize TEE: {:?} Evidence", &tee))?;
