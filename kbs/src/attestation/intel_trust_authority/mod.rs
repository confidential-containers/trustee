...
const AZURE_TDXVM_ADDR: &str = "/appraisal/v2/attest/azure/tdxvm";
...
impl Attest for IntelTrustAuthority {
    async fn verify(&self, evidence_to_verify: Vec<IndependentEvidence>) -> anyhow::Result<String> {
        ...
        for i in evidence_to_verify {
            match i.tee {
                Tee::AzTdxVtpm => {
                    att_url = format!("{}{AZURE_TDXVM_ADDR}", &self.config.base_url);

                    let evidence = from_value::<AzItaTeeEvidence>(i.tee_evidence.clone())
                        .context(format!("Failed to deserialize TEE: {:?} Evidence", i.tee))?;
                    ...
                    req_data.tdx = Some(DcapTeeEvidence {
                        ...
                        user_data: Some(STANDARD.encode(i.runtime_data.to_string())),
                        event_log: None,
                    });
                }
                Tee::Tdx => {
                    let mut evidence = from_value::<DcapTeeEvidence>(i.tee_evidence.clone())
                        .context(format!("Failed to deserialize TEE: {:?} Evidence", i.tee))?;

                    evidence.runtime_data = STANDARD.encode(i.runtime_data.to_string());
                ...
                Tee::Nvidia => {
                    let evidence = from_value::<NvDeviceEvidence>(i.tee_evidence.clone())
                        .context(format!("Failed to deserialize TEE: {:?} Evidence", i.tee))?;

                    // only one GPU supported at the moment
                    let mut nvgpu = evidence.device_evidence_list[0].clone();

                    let runtime_data_hash = Sha512::digest(i.runtime_data.to_string()).to_vec();
                    nvgpu.gpu_nonce = hex::encode(&runtime_data_hash[0..32]);

                    let decoded_evidence = STANDARD
                        .decode(nvgpu.evidence.clone())
                        .context("Failed to base64 decode GPU evidence")?;
                    nvgpu.evidence = STANDARD.encode(hex::encode(&decoded_evidence));

                    req_data.nvgpu = Some(nvgpu);
                }
                _ => {
                    bail!("Intel Trust Authority: TEE {0:?} is not supported.", i.tee);
                }
            };
        }
