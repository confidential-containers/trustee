...
impl KeylimeTeeHandler {
    ...
    pub async fn new(config: KeylimeVerifierConfig) -> Result<Self> {
        ...
        // Build the TLS identity from server ceritifcate and private key.
        let mut tls_cert_privkey = vec![];

        // Retrieve the verifier's server X509 certificate.
        let cert = {
            let path = format!("{}/server-cert.crt", config.cv_ca_path);

            fs::read(&path).context(format!(
                "unable to read Keylime server X509 certificate file from {}",
                path
            ))?
        };

        tls_cert_privkey.extend_from_slice(&cert);
        ...
        let priv_key = {
            ...
            tls_cert_privkey.extend_from_slice(&pem);
        ...
        let client = ClientBuilder::new()
            .identity(
                Identity::from_pem(tls_cert_privkey.as_slice())
