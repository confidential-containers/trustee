...
impl Provisioner {
    ...
    async fn handle_provision(&self, body: &[u8]) -> Result<Vec<u8>> {
        ...
        // SNP hostData is 32 bytes. Per the Initdata spec, when the digest is longer
        // than the TEE field, it is truncated. SHA-384 produces 48 bytes, so we take
        // the first 32 bytes to match the hostData size.
        let hostdata = {
            let digest = Sha384::digest(initdata_toml.as_bytes());
            B64.encode(&digest[..32])
        };
