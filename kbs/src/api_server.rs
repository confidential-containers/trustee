        ...
        let mut http_server = HttpServer::new({
            move || {
                ...
                App::new()
                    ...
                    .service(
                        web::resource([kbs_path!("{plugin}{subpath:.*}")])
...
pub(crate) async fn api(
    ...
    log::warn!("Policy data: {}", policy_data);
    ...
    match plugin {
        ...
        "reference-value" if request.method() == Method::GET => {
            core.admin.validate_admin_token(&request)?;
            let reference_value_id = subpath.trim_start_matches('/');
        ...
        "request-policy" if request.method() == Method::POST => {
            core.admin.validate_admin_token(&request)?;
        ...
        // TODO: consider to rename the api name for it is not only for
        // resource retrievement but for all plugins.
        "request-policy" if request.method() == Method::GET => {
            core.admin.validate_admin_token(&request)?;
        ...
        plugin_name => {
            ...
            if plugin
                .validate_auth(&body, &query, subpath, request.method())
                .await
                .map_err(|e| Error::PluginInternalError { source: e })?
            {
                ...
                core.admin.validate_admin_token(&request)?;
                let response = plugin
                    .handle(&body, &query, subpath, request.method())
                    .await
                    .map_err(|e| Error::PluginInternalError { source: e })?;
                ...
                let response = plugin
                    ...
                    .map_err(|e| Error::PluginInternalError { source: e })?;
                if plugin
                    .encrypted(&body, &query, subpath, request.method())
                    .await
                    .map_err(|e| Error::PluginInternalError { source: e })?
                {
