...
impl JwtVerifier {
    ...
    /// * `insecure_public_key_from_uri` - Whether to allow plaintext HTTP when loading key material
    ...
    pub async fn new(
        ...
        for path in trusted_cert_paths {
            ...
            let stack = X509::stack_from_pem(&cert_content)
                .with_context(|| format!("failed to parse PEM stack in {path}"))?;
            if stack.is_empty() {
