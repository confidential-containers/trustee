// Copyright (c) 2026 by Alibaba.
// Licensed under the Apache License, Version 2.0, see LICENSE for details.
// SPDX-License-Identifier: Apache-2.0

use anyhow::{bail, Context, Result};
use kbs_types::{Tee, TeePubKey};
use openssl::{
    pkey::{PKey, Private, Public},
    sign::{Signer, Verifier},
};
use serde::{Deserialize, Serialize};
use serde_json::Value;
use sigstore_types::{DsseEnvelope, DsseSignature, KeyId, PayloadBytes, SignatureBytes};
use tracing::warn;

pub const TRUST_CONTEXT_PAYLOAD_TYPE: &str =
    "https://github.com/confidential-containers/kbs/trust-context";

/// Trust Context is an overall abstraction upon different backend
/// attestation services. This lets KBS keep a single policy to filter
/// heterogeneous attestation results. The context is a general
/// representation of the attestation result for a given TEE env and is
/// used as a "passport" to access different KBS plugins.
#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct TrustContext {
    pub attestation_summary: AttestationSummary,
    pub tee_pubkey: TeePubKey,
    pub custom_claims: Value,
}

/// The backend-agnostic abstraction of an attestation result. This hides the
/// different token formats of backend attestation services and provides a
/// unified interface to the KBS policy engine and plugins.
#[derive(Default, Serialize, Deserialize, Clone, Debug)]
pub struct AttestationSummary {
    /// The TEE type(s) that the evidence has been attested for.
    pub tee_type: Vec<Tee>,

    /// The attestation policy identifiers that have been evaluated.
    pub policy_ids: Vec<String>,

    /// Identity of the attestation service that issued this result, taken from
    /// the verified token (typically its `iss` claim). This distinguishes
    /// different operators of the same kind of attestation service and is
    /// cryptographically bound to the token signature.
    pub issuer: Option<String>,

    /// Whether the backend attestation service affirmed the evidence.
    pub verification_result: bool,

    /// The full set of claims carried by the backend attestation token. KBS
    /// only requires `verification_result` for the common allow/deny decision,
    /// but the raw claims are retained here so that advanced policies can drill
    /// down into TEE-specific details (e.g. measurements or TCB status) without
    /// having to understand each backend's token format.
    pub claims: Value,
}

/// A [`TrustContext`] wrapped in a signed DSSE envelope. This is the portable
/// form that can be handed to a client and later presented back to KBS (e.g.
/// via the `Authorization` header) as a passport.
#[derive(Serialize, Deserialize)]
pub struct SignedTrustContext(DsseEnvelope);

impl SignedTrustContext {
    pub fn new(trust_context: &TrustContext, private_key: &PKey<Private>) -> Result<Self> {
        let payload = serde_json::to_vec(trust_context)?;
        let mut signer = Signer::new_without_digest(private_key)?;
        // Ed25519 (and other pure EdDSA keys) do not support streaming, so use
        // the one-shot signing API which works for both digest-less and
        // digest-based keys.
        let sig = signer.sign_oneshot_to_vec(&payload)?;
        let dsse_signature = DsseSignature {
            sig: SignatureBytes::from(sig),
            keyid: KeyId::default(),
        };
        let dsse_envelope = DsseEnvelope::new(
            TRUST_CONTEXT_PAYLOAD_TYPE.to_string(),
            PayloadBytes::from(payload),
            vec![dsse_signature],
        );
        Ok(Self(dsse_envelope))
    }

    pub fn verify(&self, public_key: &PKey<Public>) -> Result<TrustContext> {
        let payload = self.0.payload.as_bytes();
        let signature = self
            .0
            .signatures
            .first()
            .context("Signed trust context carries no signature")?
            .sig
            .as_bytes();
        let mut verifier = Verifier::new_without_digest(public_key)?;
        if !verifier.verify_oneshot(signature, payload)? {
            bail!("Trust context signature verification failed");
        }
        let trust_context = serde_json::from_slice(payload)?;
        Ok(trust_context)
    }
}

#[derive(Debug, Clone, Deserialize, PartialEq, Default)]
pub struct TrustContextConfig {
    /// URIs (file:// and https:// schemes accepted) pointing to a public key
    /// file used to verify incoming signed trust contexts.
    #[serde(default)]
    pub public_keys: Vec<String>,

    /// Path to the private key file used to sign trust contexts.
    pub private_key: Option<String>,
}

/// Signs and verifies [`TrustContext`]s using the configured key material. If
/// no keys are configured, an ephemeral key pair is generated so that trust
/// contexts issued within this process can still be round-tripped.
pub struct TrustContextManager {
    public_keys: Vec<PKey<Public>>,
    private_key: PKey<Private>,
}

impl TrustContextManager {
    pub fn new(config: &TrustContextConfig) -> Result<Self> {
        let (public_keys, private_key) = match (config.public_keys.is_empty(), &config.private_key)
        {
            (false, Some(private_key)) => {
                let public_keys = config
                    .public_keys
                    .iter()
                    .map(|public_key| {
                        let public_key_pem =
                            load_public_key(public_key).context("Failed to read public key")?;
                        PKey::public_key_from_pem(&public_key_pem)
                            .context("Failed to parse public key")
                    })
                    .collect::<Result<Vec<PKey<Public>>>>()?;

                let private_key_pem =
                    std::fs::read(private_key).context("Failed to read private key")?;
                let private_key = PKey::private_key_from_pem(&private_key_pem)
                    .context("Failed to parse private key")?;
                (public_keys, private_key)
            }
            (true, None) => {
                warn!("No public or private key provided for trust context. Generating an ephemeral key pair to sign and verify the trust context.");
                let private_key = PKey::generate_ed25519()?;
                let public_key_pem = private_key.public_key_to_pem()?;
                let public_key = PKey::public_key_from_pem(&public_key_pem)
                    .context("Failed to parse ephemeral public key")?;
                (vec![public_key], private_key)
            }
            _ => {
                bail!("Please provide either both public and private keys or neither (an ephemeral key pair will be used to sign and verify the trust context)")
            }
        };
        Ok(Self {
            public_keys,
            private_key,
        })
    }

    pub fn sign(&self, trust_context: &TrustContext) -> Result<SignedTrustContext> {
        SignedTrustContext::new(trust_context, &self.private_key)
    }

    pub fn verify(&self, signed_trust_context: SignedTrustContext) -> Result<TrustContext> {
        for public_key in &self.public_keys {
            if let Ok(trust_context) = signed_trust_context.verify(public_key) {
                return Ok(trust_context);
            }
        }
        bail!("Failed to verify signed trust context with any of the configured public keys");
    }
}

fn load_public_key(public_uri: &str) -> Result<Vec<u8>> {
    if let Some(path) = public_uri.strip_prefix("file://") {
        let public_key_pem = std::fs::read(path).context("Failed to read public key file")?;
        Ok(public_key_pem)
    } else if public_uri.starts_with("https://") {
        let public_key_pem = reqwest::blocking::get(public_uri)
            .with_context(|| format!("Failed to get public key from URL: {public_uri}"))?
            .bytes()
            .with_context(|| format!("Failed to read public key from URL: {public_uri}"))?
            .to_vec();
        Ok(public_key_pem)
    } else {
        bail!("Invalid public key URI: {public_uri}");
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    fn sample_trust_context() -> TrustContext {
        let tee_pubkey = TeePubKey::RSA {
            alg: "RSA".to_string(),
            k_mod: "test-modulus".to_string(),
            k_exp: "AQAB".to_string(),
        };
        TrustContext {
            attestation_summary: AttestationSummary {
                tee_type: vec![Tee::Sample],
                policy_ids: vec!["default".to_string()],
                issuer: Some("test-issuer".to_string()),
                verification_result: true,
                claims: Value::default(),
            },
            tee_pubkey,
            custom_claims: Value::default(),
        }
    }

    #[test]
    fn sign_and_verify_round_trip() {
        let manager = TrustContextManager::new(&TrustContextConfig::default()).unwrap();
        let trust_context = sample_trust_context();

        let signed = manager.sign(&trust_context).unwrap();
        let verified = manager.verify(signed).unwrap();

        assert_eq!(
            verified.attestation_summary.verification_result,
            trust_context.attestation_summary.verification_result
        );
        assert_eq!(
            verified.attestation_summary.tee_type,
            trust_context.attestation_summary.tee_type
        );
    }

    #[test]
    fn verify_rejects_foreign_signature() {
        let signer = TrustContextManager::new(&TrustContextConfig::default()).unwrap();
        let other = TrustContextManager::new(&TrustContextConfig::default()).unwrap();

        let signed = signer.sign(&sample_trust_context()).unwrap();
        assert!(other.verify(signed).is_err());
    }
}
