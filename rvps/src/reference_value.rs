...
/// as well as the value itself or a pointer to a remote value.
...
pub struct ReferenceValue {
    ...
    /// The reference value itself can be one either a local value,
    /// which is stored directly inside the enum variant or a remote
    /// value, which is not implemented yet, but will allow reference
    /// values to be fetched remotely as needed.
    pub value: ValueType,
}
...
impl ReferenceValue {
    ...
    pub fn new() -> Result<Self> {
        Ok(ReferenceValue {
            ...
            value: ValueType::Local(serde_json::Value::Null),
        })
    }
    ...
    pub fn set_local_value(mut self, value: serde_json::Value) -> Self {
        self.value = ValueType::Local(value);
    ...
    pub fn value(self) -> serde_json::Value {
        match self.value {
            ValueType::Local(v) => v,
            ValueType::Remote => todo!("Remote values not suppported yet."),
        }
    }
...
mod test {
    ...
    fn reference_value_serialize() {
        let rv = ReferenceValue::new()
            ...
            .set_local_value(serde_json::Value::String("abc".to_string()));
    ...
    fn reference_value_deserialize() {
        let rv = ReferenceValue::new()
            ...
            .set_name("artifact")
            .set_expiration(Utc.with_ymd_and_hms(1970, 1, 1, 0, 0, 0).unwrap())
            .set_local_value(serde_json::Value::String("abcd".to_string()));
