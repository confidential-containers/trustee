...
use internal::{config::Config, kbs_path, KBS_MAJOR_VERSION, KBS_PREFIX};
    ...
    pub async fn serve(&self) -> Result<()> {
        ...
        let http_server = HttpServer::new(move || {
            App::new()
                ...
                .service(web::resource(kbs_path!("auth")).route(web::post().to(api::auth)))
                .service(web::resource(kbs_path!("attest")).route(web::post().to(api::attest)))
                .service(
                    web::resource(kbs_path!("attestation-policy"))
                        .route(web::post().to(api::attestation_policy)),
                )
                .service(
                    web::resource(kbs_path!("token-certificate-chain"))
                        .route(web::get().to(api::get_token_certificate)),
                )
                .service(
                    web::resource([
                    ...
                    .route(web::get().to(api::get_resource))
                    .route(web::post().to(api::set_resource)),
                )
        });
